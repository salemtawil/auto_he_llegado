...
class CompincheFlowError(RuntimeError):
...
class CompincheSite(BaseSite):
    site_name = "compinche.io"
    _ENTRY_URL = "https://compinche.io/login"
    ...
    _POLL_MS = 150
    _ACTION_SPECS: tuple[CompincheActionSpec, ...] = (
        CompincheActionSpec("He llegado Instantaneas", ("He llegado Instantaneas", "He llegado Instantáneas"), ("he llegado instantaneas", "i arrived snapshots", "i arrived instant snapshots", "cheguei instantaneas"), (("he", "arrived", "cheguei"), ("llegado", "arrived", "cheguei"), ("instantaneas", "snapshots")), ("selfie", "ruta", "route", "rota")),
        CompincheActionSpec("He llegado", ("He llegado", "I arrived", "Cheguei"), ("he llegado", "i arrived", "cheguei"), (("he llegado", "i arrived", "cheguei"),), ("instantaneas", "snapshots", "selfie", "ruta", "route", "rota")),
        CompincheActionSpec("Selfie en ruta", ("Selfie en ruta", "Selfie on route", "Selfie em rota"), ("selfie en ruta", "selfie on route", "selfie on the way", "selfie em rota"), (("selfie",), ("ruta", "route", "way", "rota"))),
    )

    def __init__(self, browser_manager: BrowserManager | None = None, photo_service: ProcessPhotoService | None = None, selectors: CompincheSelectors | None = None) -> None:
        self._browser_manager = browser_manager or BrowserManager()
        self._photo_service = photo_service or ProcessPhotoService()
        self._selectors = selectors or CompincheSelectors()
    ...
    def _execute_pipeline(self, request: ProcessExecutionRequest, *, local_config: LocalConfig, progress_callback: ProgressCallback | None) -> SiteExecutionResult:
        ...
        try:
            self.emit_progress(
                progress_callback,
                phase="engine",
                message="creando sesion limpia para compinche.io",
            )
            self.emit_progress(
                progress_callback,
                phase="engine",
    ...
    def _perform_login(self, page: Page, *, request: ProcessExecutionRequest, progress_callback: ProgressCallback | None, timeout_ms: int) -> SiteExecutionResult | None:
        ...
        if login_outcome == "failure":
            return SiteExecutionResult(success=False, message="Login fallido: compinche.io reporto contrasena incorrecta o credenciales invalidas.", final_status="login_failed", phase="login")
    ...
    def _press_login_submit(self, page: Page) -> None:
        ...
        if submit is None:
            raise CompincheFlowError("login", "No se encontro el boton real de ingresar en compinche.io.")
        try:
            submit.scroll_into_view_if_needed(timeout=1_000)
        except Exception:
            pass
        self._wait_for_enabled(submit, timeout_ms=2_000, phase="login", error_message="El boton de ingresar sigue deshabilitado en compinche.io.")
        try:
        ...
        try:
            ...
            raise CompincheFlowError("login", "No se pudo presionar el boton real de ingresar en compinche.io.") from exc
    ...
    def _get_action_spec(self, action_name: str) -> CompincheActionSpec:
        ...
        raise RuntimeError(f"Accion de compinche.io no soportada: '{action_name}'. Opciones validas: {allowed}")

    def _click_action_card(self, page: Page, action_spec: CompincheActionSpec) -> None:
        ...
        while monotonic() < deadline:
            ...
            if best_locator is not None and best_score >= 80:
                self._click_locator_resilient(
                    best_locator,
                    phase="action_select",
                    error_message=f"No se pudo presionar la accion '{action_spec.ui_name}' en compinche.io.",
                )
                return
            self._wait_interval(page, self._POLL_MS)
        ...
        raise CompincheFlowError("action_select", f"No se encontro la accion '{action_spec.ui_name}' con variantes por idioma. Textos visibles: {seen}")

    def _wait_for_flow_root(self, page: Page, action_spec: CompincheActionSpec, *, timeout_ms: int, expected_min_frame_count: int = 0, session=None, extension_assisted: bool = False) -> FlowRoot:
        ...
        while monotonic() < deadline:
            ...
            self._wait_interval(page, self._POLL_MS)
        raise CompincheFlowError("iframe_detect", "No se detecto la siguiente pantalla despues del boton inicial. Se revisaron modal principal e iframes visibles, pero no aparecieron Cuenta prestada, carga de foto ni mensajes de bloque.", final_status="iframe_failed")
    ...
    def _find_action_modal(self, page: Page, action_spec: CompincheActionSpec) -> Locator | None:
    ...
    def _score_modal_root(self, modal: Locator | None, action_spec: CompincheActionSpec) -> int:
        if modal is None:
            return 0
    ...
    def _find_action_frame(self, page: Page, action_spec: CompincheActionSpec, *, expected_min_frame_count: int = 0) -> Frame | None:
    ...
    def _score_frame(self, frame: Frame, action_spec: CompincheActionSpec) -> int:
    ...
    def _detect_no_block(self, root: Page | Frame | Locator, *, page: Page, timeout_ms: int, baseline_frame_count: int = 0) -> str | None:
        ...
        if no_block_seen and not self._root_has_positive_flow_signal(root) and not self._has_positive_flow_signal(page, baseline_frame_count=baseline_frame_count):
            return "No hay bloque disponible en compinche.io para la accion seleccionada."
        return None
    ...
    def _press_borrowed_account(self, root: Page | Frame | Locator, *, borrowed_title: Locator | None = None) -> None:
        ...
        if borrowed_container is None:
            raise CompincheFlowError("account_select", "Se encontro 'Cuenta prestada', pero no se pudo resolver su contenedor clickable padre.")
        ...
        if self._try_select_borrowed_account(borrowed_container, borrowed_control):
            if self._wait_for_borrowed_account_selected(root, borrowed_container, borrowed_control=borrowed_control, timeout_ms=2_500):
                return
            raise CompincheFlowError("account_select", "Click enviado pero sin cambio real de selección en 'Cuenta prestada'.")
        raise CompincheFlowError("account_select", "Se encontro la tarjeta de 'Cuenta prestada', pero no se pudo enviar un click valido sobre su contenedor o control real.")
    ...
    def _continue_from_modal(self, root: Page | Frame | Locator, *, continue_button: Locator | None = None) -> None:
        ...
        if self._wait_for_post_continue_change(root, continue_button, timeout_ms=2_500):
            return
        raise CompincheFlowError("modal_continue", "No se detecto cambio visible despues de presionar 'Continuar'.")
    ...
    def _wait_for_block_context(
        ...
        *,
        ...
        while monotonic() < deadline:
            ...
            if no_block_message is not None:
                raise CompincheFlowError("block_read", no_block_message, final_status="no_block")
        ...
        raise CompincheFlowError(
            "block_read",
            "No aparecio la informacion del bloque despues de Continuar dentro del mismo iframe de compinche.io.",
            final_status="timeout",
        )
    ...
    def _submit_final(
        ...
        *,
        ...
        for attempt in range(1, 4):
            ...
            if button is None:
                raise CompincheFlowError("final_submit", "Se leyo el bloque, pero no aparecio el boton final REAL 'He llegado' dentro del contenedor final de compinche.io.")
            ...
            try:
                ...
                self._wait_for_enabled(
                    ...
                    timeout_ms=2_000,
                    phase="final_submit",
                    error_message="El boton final 'He llegado' sigue visible pero aun no esta habilitado en compinche.io.",
                )
                ...
                self._wait_interval(page, self._POLL_MS)
        raise CompincheFlowError("final_submit", last_error)
        ...
        while monotonic() < deadline:
            ...
            if extension_final_hint:
                ...
                return SiteExecutionResult(
                    success=True,
                    message=(
                        "Proceso completado correctamente en compinche.io. "
            ...
            if self._contains_any(root_text, self._selectors.final_failure_texts):
                raise CompincheFlowError(
                    "final_result",
                    "Se presiono el boton final, pero compinche.io devolvio un error dentro del mismo iframe antes del resultado final.",
                )
            self.emit_progress(
                progress_callback,
                phase="final_result",
    ...
    def _first_locator(self, root: Page | Frame | Locator, selectors: tuple[str, ...], *, timeout_ms: int = 2_000) -> Locator:
        ...
        for selector in selectors:
            ...
            try:
                ...
                last_error = exc
        raise RuntimeError("Ningun selector coincidio. Ajuste manual requerido para compinche.io.") from last_error
    ...
    def _wait_for_enabled(self, locator: Locator, *, timeout_ms: int, phase: str, error_message: str) -> None:
        ...
        raise CompincheFlowError(phase, error_message)
    ...
    def _click_locator_resilient(self, locator: Locator, *, phase: str, error_message: str) -> None:
        ...
        try:
            ...
            raise CompincheFlowError(phase, error_message) from exc
    ...
    def _ensure_borrowed_account_default(self, root: Page | Frame | Locator) -> None:
        ...
        borrowed_container = self._find_borrowed_account_card(borrowed_title)
        if borrowed_container is None:
            raise CompincheFlowError("account_select", "Se encontro 'Cuenta prestada', pero no su contenedor principal para validar el estado por defecto.")
        borrowed_control = self._find_borrowed_account_control(root, borrowed_container, borrowed_title)
        if borrowed_control is None:
            raise CompincheFlowError("account_select", "Se encontro 'Cuenta prestada', pero no el control real para validar que quede activa por defecto.")
    ...
    def _require_photo_input(self, root: Page | Frame | Locator, *, timeout_ms: int) -> Locator:
        ...
        if file_input is None:
            raise CompincheFlowError("photo_upload", "No se encontro el input file real dentro del frame correcto.")
        return file_input
    ...
    def _require_continue_button(self, root: Page | Frame | Locator) -> Locator:
        ...
        if continue_button is None:
            raise CompincheFlowError("modal_continue", "No se pudo localizar el boton equivalente a 'Continuar' dentro del frame correcto.")
    ...
    def _score_action_match(self, normalized_text: str, action_spec: CompincheActionSpec) -> int:
    ...
    def _press_borrowed_account(self, root: Page | Frame | Locator, *, borrowed_title: Locator | None = None) -> None:
        ...
        borrowed_control = self._find_borrowed_account_control(borrowed_container, borrowed_title)
        if borrowed_control is None:
            raise CompincheFlowError("account_select", "Se encontro la tarjeta de 'Cuenta prestada', pero no el control real asociado de seleccion.")
        own_title = self._find_best_text_candidate(root, self._selectors.own_account_texts)
        own_container = self._find_borrowed_account_card(own_title) if own_title is not None else None
        ...
        if selection_error is None:
            return
        raise CompincheFlowError("account_select", selection_error)
    ...
    def _press_borrowed_account(self, root: Page | Frame | Locator, *, borrowed_title: Locator | None = None) -> None:
        ...
        borrowed_container = self._find_borrowed_account_card(borrowed_title)
        if borrowed_container is None:
            raise CompincheFlowError("account_select", "Se encontro el texto de 'Cuenta prestada', pero no el contenedor clicable real.")
            ...
            for label in labels:
                ...
                for container in containers:
                    try:
                        ...
                        continue
        raise CompincheFlowError("block_read", f"No se pudo leer el valor del bloque para etiquetas: {', '.join(labels)}")
