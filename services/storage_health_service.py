...
class StorageHealthService:
    ...
    def get_snapshot(self) -> StorageHealthSnapshot:
        ...
        folder_usage = self._read_storage_usage_by_prefix()
        ...
        exact_used = sum(item.total_bytes for item in folder_usage)
        estimated_used = exact_used or (active_photo_count * average_file_size if average_file_size else 0)
        ...
        return StorageHealthSnapshot(
            ...
            sampled_file_count=sum(item.object_count for item in folder_usage) if folder_usage else len(sample_sizes),
            ...
            note=self._build_note(sample_sizes, configured_limit, bool(folder_usage)),
    ...
    def _read_storage_usage_by_prefix(self) -> list[StorageFolderUsage]:
        try:
            rows = self._client_provider.execute(
                self._client_provider.client.rpc("storage_usage_by_prefix")
            )
        except Exception:
            return []
        usage: list[StorageFolderUsage] = []
        for row in rows:
            if not isinstance(row, dict):
                continue
            bucket_name = str(row.get("bucket_name") or row.get("bucket_id") or "").strip()
            top_folder = str(row.get("top_folder") or row.get("prefix") or "(raiz)").strip()
            if not bucket_name:
                continue
            try:
        ...
        return sorted(usage, key=lambda item: item.total_bytes, reverse=True)
    ...
    @staticmethod
    def _build_note(sample_sizes: list[int], configured_limit: int | None, has_exact_usage: bool) -> str:
        if has_exact_usage:
            if configured_limit is None:
                return "Uso real leido desde storage.objects; configura SUPABASE_STORAGE_LIMIT_MB para calcular espacio libre."
            return "Uso real leido desde storage.objects por bucket y carpeta."
        if not sample_sizes:
