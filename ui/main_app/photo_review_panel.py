...
class PhotoReviewPanel(ctk.CTkFrame):
    ...
    def _status_summary(self, snapshot: PhotoReviewSnapshot) -> str:
        status = self.status_menu.get()
        if status == "pending":
            return f"{len(snapshot.candidates)} pendientes"
        return f"{len(snapshot.candidates)} fotos"
    ...
    def _candidate_card(self, candidate: PhotoCandidateRecord) -> ctk.CTkFrame:
        ...
        try:
            image = tk.PhotoImage(file=str(self._review_service.get_thumbnail_path(candidate)))
            self._images.append(image)
            image_label = ctk.CTkLabel(card, image=image, text="")
        except Exception:
            image_label = ctk.CTkLabel(card, text="Sin miniatura", text_color=TEXT_MUTED, height=150)
    ...
    def _bulk_action_worker(self, action: str, candidate_ids: list[str]) -> None:
        try:
            ...
            for index, candidate_id in enumerate(candidate_ids, start=1):
                ...
                self.after(0, lambda done=index, count=total, current_action=action: self._apply_bulk_progress(done, count, current_action))
                sleep(0.06)
